...
module.exports = {
    ...
    async getStatus(req, res, next) {
        ...
        var panikButtonRekap = result[4].status
        var panikButtonPublish = result[5].status
        let timeStartVote = result[1].timeStart
        let timeEndVote = result[1].timeEnd
        let timeStartPublish = result[2].timeStart
        let timeEndPublish = result[2].timeEnd
        let statusPublish = result[2].status
        ...
        if (result.length !== 0) {
            ...
            if (timeStartPublish !== null && timeEndPublish !== null ){
                if (panikButtonPublish !== true ) {
                    if (moment(dateNow).isSameOrAfter(timeStartPublish) === true && dateNow.isSameOrBefore(timeEndPublish)) {
                        // waktu sudah masuk
                    ...
                    }else if(moment(dateNow).isBefore(timeStartPublish) === true && dateNow.isBefore(timeEndPublish)){
                        // belum mu;ai 
                        await VoteStatus.findByIdAndUpdate({_id:result[2]._id},{status:false})
                    }else{
                        // sudah selesai 
                        if(lifeCircleStatus === 3){
                            await VoteLifeCicle.findByIdAndUpdate({_id:lifeCircleId},{$set:{status:4}}) 
            ...
            if (timeStartVote !== null && timeEndVote !== null) {
                if (panikButtonVote !== true ) {
                    if (moment(dateNow).isSameOrAfter(timeStartVote) === true ) {
                        try {
                         
                            if (moment(dateNow).isSameOrAfter(timeEndVote) === true && lifeCircleStatus === 1) {
                             //ketika waktu voting selesai

                                if (panikButtonRekap !== true) {
                                          // ambil semua kursi
                                ...
                                for (let index = 0; index < KursiAll.length; index++) {
                                    ...
                                    let dapil;
                                        ...
                                        if (dataCaleg.length == 0 || userLog.length == 0 || dataParpol.length == 0) {
                                            ...
                                            sit = sainteLague(resultVoteFinal, Number(kursi),{draw: true});
                                            for (var key in sit) {
                                ...
                                //update stastus ke mulai vore
                                    await VoteLifeCicle.findByIdAndUpdate({_id:lifeCircleId},{$set:{status:1}}) 
                            }
                        } catch (error) {
                            console.log(error);
                        }
                    }else if(moment(dateNow).isBefore(timeStartVote) && moment(dateNow).isBefore(timeEndVote)){
                        ...
                        //update stastus ke mulai vore
                            await VoteLifeCicle.findByIdAndUpdate({_id:lifeCircleId},{$set:{status:0}}) 
                    }
                }
            }
        } 
